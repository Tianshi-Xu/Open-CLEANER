...
class BufferedAsyncJudgeClient:
    ...
    def __init__(self, url, *, max_batch_size=1000, max_workers=4, timeout: int = 3600):
        ...
        self.timeout = timeout
        self._running = True
        self._submission_queue = None  # Initialize in __aenter__
        self._workers = None  # Initialize in __aenter__
        self._http = None  # Initialize in __aenter__

    async def __aenter__(self):
        # Create async resources in the context manager to ensure proper cleanup
        self._submission_queue = asyncio.Queue()
        self._http = aiohttp.ClientSession(base_url=self.url, timeout=aiohttp.ClientTimeout(self.timeout))
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        # Cancel and wait for all workers to finish
        if self._workers:
            for worker in self._workers:
                worker.cancel()
            await asyncio.gather(*self._workers, return_exceptions=True)
        
        # Close HTTP session if it exists and is not already closed
