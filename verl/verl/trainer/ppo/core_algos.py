...
def compute_grpo_outcome_advantage(
    ...
    Args:
        ...
        negative_mask: `(Optional[np.ndarray])`
            mask array indicating negative samples (1 for negative, 0 for positive)
            When provided, mean and std are computed only from positive samples,
            but normalization is applied to all samples
    ...
    with torch.no_grad():
        ...
        # Debug: Count negative samples
        # if negative_mask is not None:
        #     num_negative = int((negative_mask == 1).sum())
        #     num_positive = bsz - num_negative
        #     print(f"[GRPO Debug] Total samples: {bsz}, Positive: {num_positive}, Negative: {num_negative}", flush=True)
        # else:
        #     print(f"[GRPO Debug] Total samples: {bsz}, No negative mask provided", flush=True)
        
        # Collect scores, excluding negative samples when computing stats
        for i in range(bsz):
            # Only add to id2score if it's not a negative sample (for mean/std calculation)
            is_negative = negative_mask is not None and negative_mask[i] == 1
            if not is_negative:
                id2score[index[i]].append(scores[i])
        
        # Debug: Print group statistics
        # print(f"[GRPO Debug] Number of groups: {len(id2score)}", flush=True)
        # for idx in list(id2score.keys()):
        #     print(f"[GRPO Debug] Group {idx}: {len(id2score[idx])} positive samples", flush=True)
        
        # Compute mean and std from positive samples only
        for idx in id2score:
        ...
        # Debug: Print mean/std for all groups
        # print(f"[GRPO Debug] Mean/Std for all groups:", flush=True)
        # for idx in sorted(id2score.keys()):
        #     print(f"  Group {idx}: mean={id2mean[idx]:.4f}, std={id2std[idx]:.4f}, n_samples={len(id2score[idx])}", flush=True)
        
        # Apply normalization ONLY to positive samples
        # Negative samples get advantage of 0 and will only be adjusted later by DPO
        for i in range(bsz):
            is_negative = negative_mask is not None and negative_mask[i] == 1
            if is_negative:
                # Negative samples: set advantage to 0
                # They will only be adjusted later by DPO advantage adjustment
                scores[i] = 0.0
            else:
                # Positive samples: apply GRPO normalization
...
def compute_policy_loss_cispo(
    ...
    clip_ratio_low = config.clip_ratio_low if config.clip_ratio_low is not None else config.clip_ratio
    clip_ratio_high = config.clip_ratio_high if config.clip_ratio_high is not None else config.clip_ratio
    # print("---------------use CISPO policy loss---------------")
    # print("clip_ratio_low:", clip_ratio_low, "clip_ratio_high:", clip_ratio_high)
