...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    #CLEANER: add acc metric to avoid absolute score comparison, only for positive samples
    # Get negative sample mask if available
    is_negative = batch.non_tensor_batch.get("__is_negative__", None)
    if is_negative is not None:
        # Convert to boolean tensor
        negative_mask = torch.tensor(is_negative, dtype=torch.bool, device=sequence_score.device)
        # Create mask for positive samples (not negative and not aborted)
        positive_non_aborted_mask = non_aborted_mask & (~negative_mask)
        if positive_non_aborted_mask.any():
            positive_sequence_score = sequence_score[positive_non_aborted_mask]
            score_acc = torch.mean((positive_sequence_score > 0).float()).detach().item()
        else:
            score_acc = 0.0  # No positive samples
        # print("negative samples found, compute accuracy on positive samples only.")
        # print("score_acc:", score_acc)
    else:
        # Fallback: if no negative flag, compute on all non-aborted samples
        score_acc = torch.mean((non_aborted_sequence_score > 0).float()).detach().item()
    ...
    for data_source, uid2var2vals in data_src2uid2var2vals.items():
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                ...
                # Skip if no valid values remain
                if not var_vals_filtered:
                    continue
                
                # Skip string variables (check after filtering None values)
                if isinstance(var_vals_filtered[0], str):
                    continue
                ...
                if n_resps > 1:
                    ...
                    for n in ns:
                        ...
                        if var2vals.get("pred", None) is not None:
                            # Filter pred values corresponding to non-None var_vals
                            pred_vals = var2vals["pred"]
                            vote_data = [
                                {"val": var_vals[i], "pred": pred_vals[i]} 
                                for i in range(len(var_vals)) 
                                if var_vals[i] is not None
                            ]
