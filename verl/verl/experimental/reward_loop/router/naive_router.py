...
def run_router(router_ip: str, router_port: int, worker_urls: list[str]):
    ...
    def signal_handler(sig, frame):
        ...
        if router.client and not router.client.closed:
            import asyncio
            try:
                loop = asyncio.get_event_loop()
                if loop.is_running():
                    loop.create_task(router.client.close())
                else:
                    loop.run_until_complete(router.client.close())
    ...
    def _cleanup_sync(self):
        ...
        if self.client and not self.client.closed:
            try:
                ...
                # Suppress errors during cleanup to avoid breaking exit
                if self.verbose:
                    logger.warning(f"[router] Error during atexit cleanup: {e}")
