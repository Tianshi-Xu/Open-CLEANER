...
class CodeJudgeTool(BaseTool):
    def __init__(self, config: dict, tool_schema: OpenAIFunctionToolSchema):
        ...
        self.tool_session = aiohttp.ClientSession(connector=tool_connector, timeout=tool_timeout)
        ...
        # Register cleanup to ensure session is closed even if cleanup() is not called
    ...
    def _cleanup_at_exit(self):
        """Cleanup called by atexit - must be synchronous"""
        ...
        try:
            ...
            # Stop request processor if running
            if hasattr(self, 'request_processor'):
                running = getattr(self.request_processor, '_running', False)
                if running:
                    try:
                        loop.run_until_complete(self.request_processor.stop())
                    except Exception:
                        pass
            
            # Close session if not closed
            if hasattr(self, 'tool_session'):
                closed = getattr(self.tool_session, 'closed', True)
                if not closed:
                    try:
                        loop.run_until_complete(self.tool_session.close())
                    except Exception:
                        pass
    ...
    async def cleanup(self):
        """Close aiohttp session and stop request processor"""
        if self._cleanup_done:
            return
        
        if hasattr(self, 'request_processor') and self.request_processor._running:
            await self.request_processor.stop()
        if hasattr(self, 'tool_session') and not self.tool_session.closed:
