...
async def call_long_batch(
    ...
    while submissions and attempt_count < max_retries:
        ...
        try:
            ...
            # Re-raise CancelledError to let the caller know the task was cancelled
            print(f"Attempt {attempt_count}: Task was cancelled")
            raise
        except aiohttp.ClientResponseError as e:
            print(f"Attempt {attempt_count}: Server responded with {e.status}")
        except (aiohttp.ClientError, asyncio.TimeoutError, OSError) as e:
            # Added OSError to catch "Bad file descriptor" and other network-related errors
            print(f"Attempt {attempt_count}: Caught {type(e).__name__}: {repr(e)}")
        except Exception as e:
            print(f"run_tool_calls_on_server_async Error: {e}")
            traceback.print_exc()
        finally:
            # Only sleep if there are still submissions to retry
            if submissions:
...
class PersistentExecutor:
    def __init__(self):
        # Initialize with current module globals to include dynamically imported libraries
        self.exec_globals = globals().copy()
...
# Available imports mapping (same as python_executor.py)
AVAILABLE_IMPORTS = {
    'math': 'import math',
    ...
    'solve': 'from sympy import solve',
    'simplify': 'from sympy import simplify',
    'expand': 'from sympy import expand',
    'factor': 'from sympy import factor',
    'diff': 'from sympy import diff',
    'integrate': 'from sympy import integrate',
}

def analyze_required_imports(code: str) -> set:
    """Analyze user code to determine required imports"""
    import ast
    ...
    try:
        ...
        # Map used names to required imports
...
def combine_code_template(code_to_execute: str, history_code_to_execute: Optional[List[str]] = None) -> str:
    """
    Combine code template with history code and current code.
    AST analysis and auto-import optimization is DISABLED for performance.
    """
    history_code_to_execute = history_code_to_execute or []
    
    # Build final code without dynamic import analysis (disabled for performance)
    final_code = code_template_setup.replace('{dynamic_imports}', '')  # No auto-imports
